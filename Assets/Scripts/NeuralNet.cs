...
public class NeuralNet {
    ...
    public List<double> cycle (List<double> inputs) {
        ...
        for (int i = 0; i < numHiddenLayers + 1; i++) {
            ...
            // Debug.Log ("nn >> cycle >> 1 >> " + i + "/" + numHiddenLayers);
            ...
            // Debug.Log ("nn >> cycle >> 2 >> " + inputs + " :: " + inputs.Count);

            for (int j = 0; j < vecLayers[i].NumNeurons; j++) {

                // Debug.Log ("nn >> cycle >> 3 >> " + j + "/" + vecLayers[i].NumNeurons);

                double netinputs = 0;
                int f_NumInputs = 1 + vecLayers[i].vecNeurons[j].numInputs;
                cWeight = 0;

                // Debug.Log ("nn >> cycle >> 4 >> " + f_NumInputs);

                for (int k = 0; k < f_NumInputs - 1; k++) {
                    // Debug.Log ("nn >> cycle >> 5 >> " + k + "/" + f_NumInputs + " :: " + cWeight);
                    // Debug.Log ("nn >> cycle >> 5.2 >> " + inputs[cWeight]);
